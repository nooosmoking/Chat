...
public class Messaging implements Command{
    ...
    private final Map<Integer, Supplier<?>> commandMap = new HashMap<>();

    public Messaging(MessageService messageService, RoomService roomService, List<Chatroom> roomList) {
        this.messageService = messageService;
        this.roomService = roomService;
        this.roomList = roomList;
        commandMap.put(1, () -> createRoom());
        commandMap.put(2, () -> chooseRoom());
    }
...
//        out.flush();
    ...
    private void chooseRoom(){
        roomService.chooseRoom(user, roomList);
    }

    private void createRoom(){
        roomService.createRoom(roomName, user, roomList);
    }
